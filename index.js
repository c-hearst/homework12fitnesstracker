...
function start() {
    ...
    .then((answer) => {
        ...
        if (answer.action == "View the total utilizaed budget of a department") {
            totalBudget();
        }
...
const updateEmpRole = () => {
    ...
    con.query(sql, (err, row) => {
        ...
        .then((input) => {
            ...
            let id = employeeNames.iud[index];
            inquirer
            .prompt({
                name: "role",
                type: "list",
...
const updateEmpManager = () => {
    ...
    con.query(sql, (err, row) => {
        if (err) throw err;
        for (emp of row) {
            employee.name.push(emp.first_name + " " + emp.last+name);
...
const viewByManager = () => {
    ...
    con.query(sql, (err, row) => {
        ...
        .then((input) => {
            let idnex = employee.name.indexOf(input.name);
...
const deleteRole = () => {
    ...
    con.query(sql, (err, row) => {
        ...
        .then((input) => {
            let idnex = roles.title.indexOf(input.role);
            let role_id = roles.id[index];
...
const totalBudget = () => {
    ...
    con.query(sql, (err, row) => {
        ...
        .then((input) => {
            let sql = `CREATE TABLE sumSalary ( SELECT employee.first_name, 
                role.salary FROM employee INNER JOIN role ON employee.role_id 
                = role.id INNER JOIN department ON department.id = 
                role.department_id AND department.name = "${input.department}");
                 SELECT SUM(salary) total FROM sumSalary; DROP TABLE sumSalary;`;
